...
function normalizeOp(op) {
  return {
    ...op,
    id: op.id ? String(op.id) : "",
    fechaCompra: op.fechaCompra ? String(op.fechaCompra).slice(0, 10) : "",
    fechaVenta: op.fechaVenta ? String(op.fechaVenta).slice(0, 10) : "",
    marca: op.marca || "",
    modelo: op.modelo || "",
    mecanismo: op.mecanismo || "",
    proveedor: op.proveedor || "",
    precioCompra: op.precioCompra === "" || op.precioCompra === null ? "" : Number(op.precioCompra),
    gastoExtra: op.gastoExtra === "" || op.gastoExtra === null ? "" : Number(op.gastoExtra),
    canalVenta: op.canalVenta || "",
    precioVenta: op.precioVenta === "" || op.precioVenta === null ? "" : Number(op.precioVenta),
    envio: op.envio === "" || op.envio === null ? "" : Number(op.envio),
    comision: op.comision === "" || op.comision === null ? "" : Number(op.comision),
    beneficioNeto: op.beneficioNeto === "" || op.beneficioNeto === null ? null : Number(op.beneficioNeto),
    roi: op.roi === "" || op.roi === null ? null : Number(op.roi),
    vendido: op.vendido === true || String(op.vendido).toLowerCase() === "true",
    facturaNum: op.facturaNum || "",
...
export default function TimelabMobile() {
  ...
  useEffect(() => {
    async function loadOps() {
      try {
        ...
        if (json.ok) {
          const normalized = json.data.map(normalizeOp);
          setOps(normalized);
  ...
  const saveOp = async () => {
    if (!form.marca || !form.precioCompra) {
      alert("Marca y precio requeridos");
      return;
    }
    ...
    try {
      ...
      if (reloadJson.ok) {
        const normalized = reloadJson.data.map(normalizeOp);
        setOps(normalized);
        localStorage.setItem(STORAGE_KEY, JSON.stringify(normalized));
      }
      ...
      console.error(err);
      alert("No se pudo actualizar en Google Sheets");
    }
  };
